...
namespace ApiFinanzas.Controllers
{
    ...
    public class CuentasController : ControllerBase
    {
        ...
        public async Task<ActionResult<RespuestaAuthenticacion>> Registrar(UserRegister userRegister)
        {
            ...
            {
                // Verificar si el rol "User" existe, y si no, crearlo
                var userRole = "User";
                if (!await roleManager.RoleExistsAsync(userRole))
                {
                    ...
                    {
                        return BadRequest("Error al crear el rol 'User'.");
                    }
                }

                // Asignar el rol "User" al usuario
                await userManager.AddToRoleAsync(usuario, userRole);
